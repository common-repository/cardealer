...
function CarDealer_contact_form()
{
     ...
     //  'includes/contact-form/js/multi-contact-form.js', array('jquery'));
        ...
        wp_enqueue_script(
            'contact-form-js',
            CARDEALERURL . 'includes/contact-form/js/multi-contact-form.js',
            array('jquery'),
            CARDEALERVERSION
        );
...
function cardealer_process_form()
{
    ...
    define("CARDEALERRECIPIENT_NAME", "WordPress");
    define("CARDEALEREMAIL_SUBJECT", "Visitor Message From CarDealer Plugin About: " . $Car_name);
    $success = false;
    ...
    if ($senderName && $senderEmail && $message && $recipient_email) {
        $recipient = CARDEALERRECIPIENT_NAME . " <" . $recipient_email . ">";
        ...
        if (trim(get_option('CarDealer_enable_contact_form', 'no') == 'yes'))
            $success = wp_mail($recipient_email, CARDEALEREMAIL_SUBJECT, $message, $headers);
