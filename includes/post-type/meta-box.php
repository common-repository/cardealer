...
update_option('meta_boxes', $cardealer_meta_box);
function cardealer_listing_add_box()
{
    global $cardealer_meta_box;
    foreach ($cardealer_meta_box as $post_type => $value) {
...
{
    global $cardealer_meta_box, $cardealer_afeatures, $post;

    //wp_enqueue_style('meta', CARDEALERURL . 'includes/post-type/meta.css');
 
    wp_enqueue_style(
        'meta',
        CARDEALERURL . 'includes/post-type/meta.css',
        array(),
        CARDEALERVERSION  
    );
    ...
    /*
            ...
            */
			...
			<ul class="metabox-tabs" id="metabox-tabs">
				<li class="active Specs"><a class="active" href="javascript:void(null);"><?php echo esc_attr__('Specs', 'cardealer') ?></a></li>
				<li class="Features"><a href="javascript:void(null);"><?php echo esc_attr__('Features', 'cardealer') ?></a></li>
			</ul>
			<div class="Specs">
				<h4 class="heading"><?php echo esc_attr__('Specs', 'cardealer') ?></h4> 
    <?php
    echo '<input type="hidden" name="listing_meta_box_nonce" value="',
       esc_attr(wp_create_nonce(basename(__file__))), '" />';
    foreach ($cardealer_meta_box[$post->post_type]['fields'] as $field) {
        ...
        switch ($field['type']) {
            case 'text':
                ...
                echo '<div class="box-label"><label for="' . esc_attr($field['id']) . '">' . esc_attr($title) =
                    esc_attr(str_replace("_", " ", $title)) . '</label></div>';
*/
                ...
                echo '<input type="text" name="' . esc_attr($field['id']) . '" class="' . esc_attr($field['name']) .
                    '" id="' . esc_attr($field['id']) . '" value="' . esc_attr($meta ? $meta : esc_attr($field['default'])) .
            ...
            case 'checkbox':
                echo '<div class="boxes-small">' . '<div class="box-label"><label for="' . esc_attr($field['id']) .
                    '">' . esc_attr($title = str_replace("_", " ", esc_attr($title))) . '</label></div>' .
                    '<div class="box-content"><p>';
                ...
                echo '<input type="checkbox" class="' . esc_attr($field['name']) .
                    ...
                    ' checked="checked"' : '') . '<br />' . esc_attr($field['desc']);
                echo '</div>';
                echo '</div></div>';
                break;
        } // end Switch
  ...
  if( empty($cardealer_features))
    {
       echo '<br />';
       echo '<br />';
       echo  esc_attr__('Add any Feature Field (Settings => Field Features)', 'cardealer');
       echo '<br />';
       echo '<br />';    
   }
    else
    {
        ...
        echo '<h4 class="heading">'.esc_attr__('Features', 'cardealer').'</h4>';
    	...
    	for($i=0; $i < $qnew; $i++)
        {
    		...
    		'<div class="box-label"><label for="'. esc_attr($field_name) .'">'. esc_attr($title = str_replace("_", " ",$title)) . '</label></div>'.
    		'<div class="box-content"><p>';
            echo '<input type="text" name="'. esc_attr($field_id). '" class="'. esc_attr($field_name) .'" id="'. esc_attr($field_id).'" value="'. esc_attr($meta). '" size="30" style="width:97%" />'. '<br />'. '';
            echo '</div> </div>';
	}
...
{
    global $current_post_id, $cardealer_meta_box, $post, $cardealer_afeatures;
    $current_post_id = $post_id;
    if (!is_object($post))
        return;
    if (!isset($cardealer_meta_box[$post->post_type]['fields'])) {
        return;
    }
    //Verify nonce
