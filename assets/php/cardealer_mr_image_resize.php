 ...
 *
 *  @author Ernesto M�ndez 
 *  @author Matthew Ruddy 
 */
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly 
add_action('delete_attachment', 'cardealer_mr_delete_resized_images');
function cardealer_mr_image_resize($url, $width = null, $height = null, $crop = true, $align =
    'c', $retina = false)
{
    global $wpdb;
    // Get common vars (func_get_args() only get specified values)
    $common = cardealer_mr_common_info($url, $width, $height, $crop, $align, $retina);
...
function cardealer_mr_common_info($url, $width, $height, $crop, $align, $retina)
{
    ...
    if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
        ...
        $file_path = sanitize_text_field(ABSPATH . str_replace(dirname(sanitize_text_field($_SERVER['SCRIPT_NAME'])) . '/', '',
...
function cardealer_mr_delete_resized_images($post_id)
{
